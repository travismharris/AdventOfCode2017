...
namespace Day2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var inputAsListOfIntArray = LoadPuzzle(@"..\Input\Day2Input.txt");
            var returnList = GetDifferences(inputAsListOfIntArray);
            Console.Write("Checksum = " + returnList.Sum());

            var newReturnList = GetModuloZeroDivisionResults(inputAsListOfIntArray);
            Console.WriteLine("\nNew Checksum = " + newReturnList.Sum());
            Console.ReadKey();
        }

        public static List<int[]> LoadPuzzle(string fileName)
        {
            ...
            {
                var a = line.Split();
                var subIntArray= Array.ConvertAll(a, int.Parse);
                intArray.Add(subIntArray);
            }
            return intArray;
        }

        public static List<int> GetDifferences(List<int[]> array)
        {
            List<int> returnList = new List<int>();
            foreach( var a in array)
            {
                returnList.Add(a.ToList().Max() - a.ToList().Min());
            }
            return returnList;
        }
        
        public static List<int> GetModuloZeroDivisionResults(List<int[]> arrays)
        {
            ...
            foreach(var list in lists)
            {
                ...
                for(int i = 0; i < list.Count; i++)
                {
                    for(int j = 1; j<list.Count; j++)
                    {
                        if (i == j)
                            continue;
                        if (list[i] % list[j] ==0 || list[j]% list[i] ==0)
                        {
                            if (list[i] > list[j])
